...
extra_libs <- c(.Library.site, .Library)
extra_libs <- extra_libs[nzchar(extra_libs) & dir.exists(extra_libs)]
.libPaths(unique(c(.libPaths(), extra_libs)))
...
debtrivedi_path <- c(
  "DebTrivedi.rda",
  file.path("vignettes", "DebTrivedi.rda"),
  system.file("doc", "DebTrivedi.rda", package = "pscl")
)
debtrivedi_path <- debtrivedi_path[file.exists(debtrivedi_path)][1]

if(is.na(debtrivedi_path)) stop("Could not locate DebTrivedi.rda")
load(debtrivedi_path)
...
at \url{https://CRAN.R-project.org/package=pscl}.
...
\url{http://qed.econ.queensu.ca/jae/1997-v12.3/deb-trivedi/}. It was
